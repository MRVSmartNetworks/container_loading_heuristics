...
class Solver23():
    ...
    def solve(self, df_items, df_vehicles, sol_file_name, time_limit=30*60):
        ...
        for iter in range(N_SOL_ITER):
            ...
            if TEST_PAR == False:
                df_sol, tot_cost, time_spent = self.solver(df_items, df_vehicles, time_limit)
            ...
            while TEST_PAR:
                ...
                f.write(f"ALPHA = {ALPHA}   BETA = {BETA}   Avg_cost = {tot_cost}   std = {st.stdev(list_cost)}   best_value = {min(list_cost)}\n")
        ...
        if(len(iter_time) > 1):
            print(f"Average time spent by every iteration: {sum(iter_time)/len(iter_time)} s")
    ...
    def solver(self, df_items, df_vehicles, time_limit):
        ...
        while(more_items):
            ...
            if SINGLE_PLOT:
                orthogonal_plane(df_items_or, df_vehicles_or,pd.DataFrame.from_dict(self.sol), idx_vehicle=i)
                i += 1
                        ...
                        if len(unique_height) > 1:
                            ...
                            while h < len(unique_height) and not fit:
                                ...
                                if unique_height[h] + stack.height <= constraints["max_height"]:
                                    valid_items = items_code[items_code.height == unique_height[h]]
                                    i = 0
                                    while i < len(valid_items) and not fit:
                                        ...
                                        if stack.addItem(valid_items.iloc[i], constraints) == 1:
                                            ...
                                            items_code = items_code[items_code.id_item != valid_items.iloc[i].id_item]
                    ...
                    if stack_added == -2:
                        ...
                        if len(unique_weight) > 1:
                            ...
                            while w < len(unique_weight) and not fit:
                                if unique_weight[w] + stack.weight <= constraints["max_weight_stack"]:
                                    valid_items = items_code[items_code.weight == unique_weight[w]]
                                    i = 0
                                    while i < len(valid_items) and not fit:
                                        if stack.addItem(valid_items.iloc[i], constraints) == 1:
                                            fit = True
    ...
    def vehicle_decision(self, df_vehicles, items_left, area_ratio, weightRatio):
        ...
        maxIt_D = max((items_left["weight"] / items_left["width"] * items_left["height"]))
        ...
        if(Usable_vehicles.empty):
            raise ValueError("No vehicles can carry certain items due to constrain not respected")
        
        # Possible last vehicles
        Last_vehicles = Usable_vehicles[Usable_vehicles.max_weight >= (tot_weight*1.1)]
        Last_vehicles = Last_vehicles[Last_vehicles.volume >= (tot_vol*1.25)] # *1.25 is to have some margin

        # Best efficiency vehicles
        BestEff_vehicles = Usable_vehicles.sort_values(by=["tot_eff_ratio"], ascending = False).reset_index()
        BestWeight_vehicles = Usable_vehicles.sort_values(by=["weight_eff"], ascending = False).reset_index()

        # Case in which some vehicles can carry all the items remained
        if(Last_vehicles.empty == False):
            Last_vehicles = Last_vehicles.sort_values(by=["cost"]) # Little cost choosen
            last_iter = True
            ...
            if weightRatio > 0.95 and area_ratio < 0.95:
                if ((BestEff_vehicles.at[0,"area"] -  BestWeight_vehicles.at[0,"area"]) < (1-area_ratio) * BestEff_vehicles.at[0,"area"]):
