        ...
        while _iter < self.n_iter and not good_sol:
            ...
            for ant_index in range(self.n_ants):
                ...
                while free_space:
                    ...
                    new_stack, stack_lst_ant, stack_quantity_ant = self.popStack(
                        stack_lst_ant, stack_quantity_ant, next_s_code
                    )
                    toAddStack, first_line, x_pos = self.addStack(
                        new_stack, ant_k[:n_1_line], first_line, x_pos
                    )
                    if first_line:
                        n_1_line = len(ant_k) + 1
                    # Check if a stack can be added due tue the vehicle weight constrain
                    if toAddStack is not None and (
                        totWeight + toAddStack.weight <= self.vehicle["max_weight"]
                    ):
    ...
    def addStack(self, toAddStack, stack_1_line, first_line, x_pos):
        ...
        if first_line:
            ...
            if (
                ...
                toAddStack.position(
                    x_pos, 0
                )  # Assign coordinates to the stack (if fitting)
                x_pos += toAddStack.length
                first_line = True
            else:
                first_line = False
